...
var description = "alpha version 2\nin test. things won't go so smoothly yet. extremely unbalanced.";
...
var init = () => {
  ...
  // n
  {
    ...
    n = theory.createUpgrade(3, delta, new ExponentialCost(5e6, Math.log2(6)));
  ...
  {
    bExp = theory.createMilestoneUpgrade(0, 11);
...
var resetStage = () => { a.level = 0; b.level = 0; c.level = 0; delta.value = BigNumber.ONE; currency.value = BigNumber.ZERO; theory.clearGraph(); }
var getResetStageMessage = () => 'You are about to reset this publication. Proceed?';

var getPublicationMultiplier = (tau) => tau.pow(0.1) * tau.max(BigNumber.TEN).log10().pow(0.1);
var getPublicationMultiplierFormula = (symbol) => `(${symbol} \\log ${symbol})^{0.1}`;
